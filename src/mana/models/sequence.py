...
class Sequence:
    ...
    def from_mir_file(cls,
        ...
        """Loads an sequence .json file in Mocap Intel RealSense format and 
        returns an Sequence object.
    ...
    def from_mir_json(cls,
        ...
        and returns an Sequence object.
    ...
    def from_mka_json(cls, json_str: str, name: str = 'Sequence') -> 'Sequence':
        """Loads an sequence from a json string in Mocap Intel RealSense 
    ...
    def from_hdm05_c3d_file(cls,
        ...
        # * For some reason, all body part positions are the same in the last
        # * frame, so we just remove it
    ...
    def from_hdm05_asf_amc_files(cls,
        ...
        Sequence object including cody part coordinates.
    ...
    def append(self, sequence) -> 'Sequence':
        ...
        together. 
        """
        if self.positions.shape[1] != sequence.positions.shape[
                1] or self.positions.shape[2] != sequence.positions.shape[2]:
            raise ValueError(
                f'sequence.position shapes {self.positions.shape} and '
                '{sequence.positions.shape} do not fit together.\nPlease ensure'
                ' that the second and third axes share the same dimensionality.'
            )
    ...
    def split_into_batches(self, size: int = 10):
        """ Splits this sequence into batches of specified size. Returns a 
        consecutive list of sequences with length of the given size. 
        ...
        batches = [
            ...
            for i in range(0, len(self.positions), size)
        ]
        return [
            Sequence(batch, f'{self.name}__batch-{i}',
                     f'{self.desc}__batch-{i}')
    ...
    def norm_relative_to_position_idx(self, position_idx):
        self.positions = norm.relative_to_root(self.positions,
                                               root_idx=position_idx)
        return

    def norm_relative_to_positions(self, positions):
        self.positions = norm.relative_to_positions(self.positions,
